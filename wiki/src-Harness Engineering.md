...
- [[src-Claude Code Session管理]] — 工作流程層的五個分歧點選擇（continue / rewind / clear / compact / subagent）就是 Harness 的操作實踐；context rot 在 300-400k 實測觀察補強 agents.md 「不能太長」的量化依據
- [[src-agent-model-body-harness]] — 把「Harness」進一步拆為「Body（能力延伸）+ Harness（行為校準）」三層拆解；本文「能力邊界」對應該文的 Body 層，本文「認知框架/工作流程」對應該文的 Harness 層
